		...
		for (std::pair<std::string, VariableEntry*> symbol : symbolMap)
		{
			if (symbol.second->getType() == VariableEntry::GLOBAL)
			{
				...
				for (int processNumber : processes)
				{
					...
					newPredicates.push_back(Ast::newBinaryOp(Ast::newID(currentCounterName), Ast::newINT(K), literalCode::LESS_EQUALS));
					newPredicates.push_back(Ast::newBinaryOp(Ast::newID(currentCounterName), Ast::newINT(0), literalCode::GREATER_EQUALS));
					newPredicates.push_back(Ast::newBinaryOp(Ast::newID(currentFirstPointerName), Ast::newINT(K), literalCode::LESS_EQUALS));
					newPredicates.push_back(Ast::newBinaryOp(Ast::newID(currentFirstPointerName), Ast::newINT(0), literalCode::GREATER_EQUALS));
					newPredicates.push_back(Ast::newBinaryOp(Ast::newID(currentFirstPointerName), Ast::newID(currentCounterName), literalCode::LESS_EQUALS));
