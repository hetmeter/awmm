...
/* Global variable handling */
	...
	std::vector<std::string> getMinimalImplyingCubes(Ast* predicate, const std::vector<int> &relevantIndices)
	{
		...
		while (!pending.empty())
		{
			...
			currentImplication = currentCube->getPredicateImplication(predicate);
			...
			{
				currentCanonicalSupersetRepresentations = currentCube->getCanonicalSupersetStringRepresentations();
				pending.insert(pending.begin(), currentCanonicalSupersetRepresentations.begin(),
	...
	std::vector<std::string> getAllFalseImplyingCubes()
	{
		...
		{
			currentCube = getImplicativeCube(pending[0]);
			currentImplication = currentCube->getPredicateImplication(getFalsePredicate());
