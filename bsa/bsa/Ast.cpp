		...
		else
		{
				...
				|| _name == literalCode::LOCAL_ASSIGN_TOKEN_NAME)
			{
				...
				if (relevantPredicateIndices.size() > 0)
				{
					...
					{
						replacementStatements.push_back(
								newLocalAssign(config::auxiliaryTemporaryVariableNames[ctr], newINT(0))
							);
					}
...
/* Public methods */
	...
	bool Ast::unfoldIfElses()
	{
		if (_name == literalCode::IF_ELSE_TOKEN_NAME)
		{
			cout << "\tPerforming predicate abstraction on: if(" << _children[0]->getCode() << ")...\n\n";

			bool isLabeled = _parent->getName() == literalCode::LABEL_TOKEN_NAME;
			bool hasElse = _children[2]->getName() != literalCode::NONE_TOKEN_NAME;
			vector<Ast*> replacementStatements;
			vector<Ast*> currentStatements;
			Ast* conditional = _children[0];
			...
			currentStatements = _children[1]->getChildren();
			...
			if (hasElse)
			{
				...
				currentStatements = _children[2]->getChildren();
				replacementStatements.insert(replacementStatements.end(), currentStatements.begin(), currentStatements.end());
			}
			...
			if (isLabeled)
			{
				...
				{
					_parent->getParent()->insertChild(replacementStatements.at(ctr), _parent->getIndexAsChild() + 1);
				}

				config::replaceNode(firstStatement, this);
			}
			else
			{
				for (int ctr = replacementStatements.size() - 1; ctr >= 0; ctr--)
				{
					_parent->insertChild(replacementStatements.at(ctr), getIndexAsChild() + 1);
				}
		...
		{
			return _children[1]->unfoldIfElses();
		}
	...
	void Ast::topDownCascadingUnfoldIfElses()
	{
		if (_name == literalCode::STATEMENTS_TOKEN_NAME)
		{
			...
			while (hasChanged)
			{
				...
				{
					hasChanged = hasChanged || _children[ctr]->unfoldIfElses();
				}
			}
		}
		else
		...
		else
		{
				...
				_name == literalCode::FLUSH_TOKEN_NAME)
			{
				...
				if (!replacement.empty())
				{
					...
					config::prepareNodeForLazyReplacement(replacement, this);
				}
			}
			else if (_name != literalCode::INITIALIZATION_BLOCK_TOKEN_NAME &&
				_name != literalCode::BL_INITIALIZATION_BLOCK_TOKEN_NAME &&
		...
		if (_name == literalCode::BL_PROGRAM_DECLARATION_TOKEN_NAME)
		{
			...
			for (int ctr = numberOfGlobalPredicates - 1; ctr >= 0; ctr--)
			{
				_children[2]->insertChild(
						newLabel(
							config::getCurrentAuxiliaryLabel(),
							newStore(config::auxiliaryBooleanVariableNames[ctr], newAsterisk())
						),
						0
					);
			}
